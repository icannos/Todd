...
class XLMRobertaQueryScorer(QueryBasedScorer):
    def __init__(self,
    ...
    def prepare_sentence(self, sentence: str, model=None, tokenizer=None):
        assert isinstance(model, XLMRobertaForMaskedLM)
        ...
        if isinstance(sentence, tuple) and self.concat_output:
            ...
            token_type_ids = tokenizer(sentence[0], sentence[1]).sequence_ids()
        else:
            if self.prefix is not None:
    ...
    def __format__(self, format_spec):
        return f"XLMRobertaQueryScorer()"
